...
function renderBeachClosures(jsonData) {
    ...
    if (cardFooter && jsonData.length > 0) {
        ...
        let updateDate = null;
        if (jsonData[0] && jsonData[0]["Date"]) {
            // Check if a 'Date' field exists
            updateDate = dayjs(jsonData[0]["Date"]).toDate(); // Convert to JS Date
        } else {
