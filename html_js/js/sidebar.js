...
function setupSidebarHandle() {
  ...
  requestAnimationFrame(() => {
    ...
    const handlePadding =
      parseFloat(handleStyles.paddingTop) +
      parseFloat(handleStyles.paddingBottom);
    ...
    minHeight =
      ...
      sidebarHandle.getBoundingClientRect().height +
      handlePadding +
      headerPadding;
    ...
    for (const grippable of grippables) {
      ...
      grippable.addEventListener("touchstart", sidebar_startDrag, {
        passive: false,
      });
    }
  });
...
function sidebar_dragEnd() {
  ...
  const snapHeights = [minHeight, middleHeight, maxHeight].filter(
    (h) => !isNaN(h)
  );
...
function sidebar_setHeight(height = -1) {
  ...
  const heightString = `calc(${height}px - ${
    sidebarHeader.getBoundingClientRect().height
  }px - (var(--p1) * 4))`;
...
function toggleSidebar() {
  ...
  if (sidebarContents.classList.contains("open")) {
    ...
    if (mapDisclaimers)
      mapDisclaimers.style.left =
        "calc(var(--p2) + var(--sidebar-desktop-width))";
  } else {
